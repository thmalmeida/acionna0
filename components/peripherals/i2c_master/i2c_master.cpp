...
I2C_Master::I2C_Master(i2c_port_t port, gpio_num_t scl, gpio_num_t sda, uint32_t freq, bool pull_up /* = false */) : i2c_master_port_(port) {
	// Configuration
	i2c_config_t conf = {};
...
I2C_Master::~I2C_Master() {
	i2c_driver_delete(i2c_master_port_);
}
int I2C_Master::write(uint8_t slave_addr, uint8_t reg, uint8_t* data, size_t len, bool ack_check) {
	...
	ret = i2c_master_cmd_begin(i2c_master_port_, cmd_handle, I2C_COMMAND_WAIT_MS / portTICK_RATE_MS);
...
int I2C_Master::write2(uint8_t slave_addr, uint8_t *data, uint8_t data_len) {
	...
	esp_err_t ret = i2c_master_write_to_device(i2c_master_port_, slave_addr, data, data_len, I2C_COMMAND_WAIT_MS / portTICK_RATE_MS);
...
int I2C_Master::read_only(uint8_t slave_addr, uint8_t* data, size_t data_len, bool ack_check) {
	...
	i2c_master_stop(cmd_handle);
	esp_err_t ret = i2c_master_cmd_begin(i2c_master_port_, cmd_handle, I2C_COMMAND_WAIT_MS / portTICK_RATE_MS);
	i2c_cmd_link_delete(cmd_handle);
...
int I2C_Master::read2(uint8_t slave_address, const uint8_t *write_buffer, size_t write_buffer_len, uint8_t *read_buffer, size_t read_buffer_len) {
	...
	esp_err_t ret = i2c_master_write_read_device(i2c_master_port_, slave_address, write_buffer, write_buffer_len, read_buffer, read_buffer_len, I2C_COMMAND_WAIT_MS / portTICK_RATE_MS);
